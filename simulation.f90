subroutine simulation(root_dirname, dirname)
  ...
  !1-2 int
  integer :: i, j, n, nn
  ! integer :: ii ! ***formoving***
  integer :: seed
  integer :: counter = 0 !for debug

  !1-3 parameter.csv
  integer :: f_noise, f_lj, f_wlj, f_lnk, f_clsn, f_elp, f_moment0
  ...
  integer , parameter :: circle_num = 8 !number of displayed particles for pore
  ...
  double precision , parameter :: wall_x = 14.0d0 !m wall_x
  double precision , parameter :: wall_y = 18.0d0 !m wall_y
  double precision , parameter :: wall_z = 0.9d0 !m wall_z
  ...
  ! ***formoving***
  ! double precision , dimension(output_times , total_particle_num) :: idoudistances
  ! double precision , dimension(3 , total_particle_num) :: prepos !m position iti
  ...
  open(12, file = bondpath)
    ...
    !pore
    do i = 1 , circle_num - 1
      write(12,'(i3," ",i3," 0")')total_particle_num + i, total_particle_num + i+1
    end do
    write(12,'(i3," ",i3," 0")')total_particle_num + circle_num, total_particle_num+1
    !square
    write(12,'(i3," ",i3," 0")')total_particle_num + circle_num+1, total_particle_num + circle_num+2
    write(12,'(i3," ",i3," 0")')total_particle_num + circle_num+2, total_particle_num + circle_num+3
    write(12,'(i3," ",i3," 0")')total_particle_num + circle_num+3, total_particle_num + circle_num+4
    write(12,'(i3," ",i3," 0")')total_particle_num + circle_num+4, total_particle_num + circle_num+1
  close(12)
  ...
  open(21,file=filename)
    ...
    !marker baes (pore)
    do i = 1 , circle_num
      write(21,'(i4,i3,3e14.4)')total_particle_num + i , 8 , &
      poreR*cos(2.0d0*pi/dble(circle_num)*dble(i)) , poreR*sin(2.0d0*pi/dble(circle_num)*dble(i)) , 0.0d0
    end do
    !marker baes (square)
    write(21,'(i4,i3,3e14.4)')total_particle_num + circle_num + 1 , 8 , wall_x , -wall_y , 0.0d0
    write(21,'(i4,i3,3e14.4)')total_particle_num + circle_num + 2 , 8 , wall_x , wall_y , 0.0d0
    write(21,'(i4,i3,3e14.4)')total_particle_num + circle_num + 3 , 8 , -wall_x , wall_y , 0.0d0
    write(21,'(i4,i3,3e14.4)')total_particle_num + circle_num + 4 , 8 , -wall_x , -wall_y , 0.0d0
  close(21)
  ...
  do nn = 1 , output_times
    do n = 1 , output_interval
      ...
      ! if(n<30)then
      !   write(*,fmt='(I5)',advance='no')n/output_interval
      !   print *, "v^2", rho*Cd*pi*((D/2.0d0)**2.0d0)*v*pre_moment(3,1)/2.0d0 ,"clsn", &
      !   clsn(3,1), "noise", noise(3,1), "elp", elp(3,1), "force", force(3,1), &
      !   "momwnt", moment(3,1), "pos", pos(3,1)
      ! endif
    end do
    ...
    open(22,file=filename)
      ...
      !marker baes (pore)
      do i = 1 , circle_num
        write(22,'(i4,i3,3e14.4)')total_particle_num + i , 8 , &
        poreR*cos(2.0d0*pi/dble(circle_num)*dble(i)) , poreR*sin(2.0d0*pi/dble(circle_num)*dble(i)) , 0.0d0
      end do
      !marker baes (square)
      write(22,'(i4,i3,3e14.4)')total_particle_num + circle_num + 1 , 8 , wall_x , -wall_y , 0.0d0
      write(22,'(i4,i3,3e14.4)')total_particle_num + circle_num + 2 , 8 , wall_x , wall_y , 0.0d0
      write(22,'(i4,i3,3e14.4)')total_particle_num + circle_num + 3 , 8 , -wall_x , wall_y , 0.0d0
      write(22,'(i4,i3,3e14.4)')total_particle_num + circle_num + 4 , 8 , -wall_x , -wall_y , 0.0d0
    close(22)

  end do
  ! ***formoving***
  ! write(filename, '(a, a, "idou.txt")') trim(root_dirname), slash
  ! open(22,file=filename)
  !   do ii=1,output_times
  !     do i=1,total_particle_num
  !       write(22,*)idoudistances(ii, i)
  !     end do
  !   end do
  ! close(22)
